...
#if os(iOS)
private struct FixedChatHeaderOverlay: UIViewRepresentable {
    let viewer: ChatEndpoint
    let onBack: () -> Void
    let onToggleViewer: () -> Void
    ...
    func makeUIView(context: Context) -> HeaderOverlayAttachmentView {
        let view = HeaderOverlayAttachmentView()
        view.coordinator = context.coordinator
        context.coordinator.update(viewer: viewer, onBack: onBack, onToggleViewer: onToggleViewer)
        return view
    }

    func updateUIView(_ uiView: HeaderOverlayAttachmentView, context: Context) {
    ...
    static func dismantleUIView(_ uiView: HeaderOverlayAttachmentView, coordinator: Coordinator) {
        coordinator.remove()
        uiView.coordinator = nil
    }

    final class Coordinator {
        private var rootView: FixedChatHeaderContent?
        private var hostingController: UIHostingController<FixedChatHeaderContent>?
        private var overlayView: HeaderPassthroughView?
        private weak var installedWindow: UIWindow?

        func update(viewer: ChatEndpoint, onBack: @escaping () -> Void, onToggleViewer: @escaping () -> Void) {
            let rootView = FixedChatHeaderContent(
                viewer: viewer,
                onBack: onBack,
        ...
        func install(in window: UIWindow) {
            guard overlayView == nil || installedWindow !== window else {
            ...
            let hostingController = hostingController ?? UIHostingController(rootView: rootView)
            hostingController.view.backgroundColor = .clear
            hostingController.view.translatesAutoresizingMaskIntoConstraints = false
            self.hostingController = hostingController

            let overlayView = HeaderPassthroughView()
...
private final class HeaderOverlayAttachmentView: UIView {
    weak var coordinator: FixedChatHeaderOverlay.Coordinator?
...
private final class HeaderPassthroughView: UIView {
    override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        headerHitRegions().contains { $0.contains(point) }
    }

    private func headerHitRegions() -> [CGRect] {
        let screenHeight = max(bounds.height, UIScreen.main.bounds.height)
        let scale = min(bounds.width / 402, screenHeight / 874)
        let scaledWidth = 402 * scale
        ...
        return [
            rect(x: 5, y: 37, width: 54, height: 54),
            rect(x: 171, y: 62, width: 60, height: 60),
            rect(x: 161, y: 116, width: 80, height: 33)
        ]
    }
}

private struct FixedChatHeaderContent: View {
    let viewer: ChatEndpoint
    let onBack: () -> Void
    let onToggleViewer: () -> Void
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            let scale = min(proxy.size.width / 402, stableHeight / 874)
